...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        Log.v("ttttttt","ggggggggggggggggggg");
        Log.v("tttttt",remoteMessage.getData().toString());
...
/////////////////////////////////////////
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            Log.v("ttttttt","000000000000000000000");
    ...
    private void handleDataMessage(JSONObject json) {
        Log.v("ttttttt",json.toString()+"    333333");
        ...
        try {
            ...
            Log.v("ttttttt","1111111    -1111111111111");

            if (!NotificationUtils.isAppIsInBackground(getApplicationContext())) {
                //CommonMethods.DebuggableLogE(TAG, "IF: " + json.toString());
                // app is in foreground, broadcast the push message
                Log.v("ttttttt","1111111    000000000");
                if (json.getJSONObject("custom").has("ride_request")) {
                    Log.v("ttttttt","1111111    333333");
                    if (DriverStatus.equals("Online")
//                            && (TripStatus == null || TripStatus.equals(CommonKeys.TripStaus.EndTrip)
//                            || TripStatus.equals(""))
                            ) {
                        Log.v("ttttttt","1111111    22222222222222");
...
//                    ) || TripStatus.equals(""))
                ...
                } else if (json.getJSONObject("custom").has("trip_payment"))
                {
                    ...
                    NotificationUtils notificationUtils = new NotificationUtils(getApplicationContext());
                    notificationUtils.playNotificationSound();
                    String message = json.getJSONObject("custom").
                            getJSONObject("custom_message").getString("message_data");
                    String title = json.getJSONObject("custom")
                            .getJSONObject("custom_message").getString("title");

                    notificationUtils.generateNotification(getApplicationContext(), message,title);
                }
                else if (json.getJSONObject("custom").has("remember_driver")) {
                    NotificationUtils notificationUtils = new NotificationUtils(getApplicationContext());
                    notificationUtils.playNotificationSound();
                    String message = json.getJSONObject("custom").
                            getJSONObject("remember_driver").getString("message_data");
                    String title = json.getJSONObject("custom")
                            .getJSONObject("remember_driver").getString("title");

                    notificationUtils.generateNotificationForRememberDriver(getApplicationContext(), message,title);
                }
            ...
            Log.v("ttttttt","1111111    99999999999999999"+e.getMessage());
